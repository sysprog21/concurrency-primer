...
In this case, the counter can be incremented using \texttt{fetch\_add(1, memory\_order\_relaxed)} because the only requirement is atomicity;
...
Last but not least, we introduce \texttt{memory\_order\_consume}.
...
The data being accessed (\texttt{p->keyboards}) relies on the value of \texttt{p},
leading most platforms, including those with weak ordering,
to maintain the sequence of the initial load (\mintinline{cpp}{p = peripherals}) and its subsequent use (\mintinline{cpp}{p->keyboards}).
...
\end{minted}
\end{colfigure}
Writers must wait until the \texttt{readers} count drops to zero,
while readers can acquire the lock through an atomic \textsc{RMW} operation if \texttt{hasWriter} is \mintinline{cpp}{false}.
