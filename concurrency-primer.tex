...
\section{Concurrency tool and Synchronization}
...
However, this simplicity can also cause threads to pause for unpredictable durations and then influence the throughput of the overall system.
...
If the second thread holds another mutex first, 
then a thread locks the mutex and subsequently tries to lock the other mutex that is held by the second thread, 
then the deadlock occurs. 
Therefore, we can see that deadlock happens when different threads lock in incompatible orders, 
which leads to the system becoming immobilized as threads perpetually wait on each other. 
...
Consider this program: if there is only a single thread, execute these operations as follows:
...
then the value of x will always be 0, which will lead to livelock. 
Therefore, even without any locks in concurrent threads, 
they still cannot guarantee that the overall system can make progress toward achieving the programmer's goals.
...
\section{Lock Free}
In \secref{concurrency-tool}, we explored different mechanisms based on concurrency tool characteristics, as described in \secref{atomicity} and \secref{rmw}.
...
\includegraphics[keepaspectratio, width=0.4\linewidth]{images/progress-type}
\captionof{figure}{We can classify progress types into three sets: 
the set Obstruction Free includes the set Lock Free, 
and the set Lock Free includes the set Wait Free. 
Achieving Wait Free is the most optimal approach, 
allowing each thread to progress without being blocked by others.}
...
which may involve utilizing shared resources for communication (e.g., spinlock), as shown in \secref{shared-resources}. 
...
and the consumers take tasks from the job queue and execute them.
...
\textbf{state 1} : The producer is adding tasks to the job queue while consumers wait for tasks to become available and is ready to take on any job that appears in the job queue.
...
This mechanism is an advanced form of mutex lock.
...
it causes consumers to have no work available, 
leading them to block and thus halting progress in the entire system. 

Secondly, concurrently, consumers need to access shared resources, which is the job.
...
To solve this issue, the introduction of lock-based and lock-free algorithm is presented in the following section.
...
there is guaranteed work available unless all jobs have been taken by other consumers.
...
The algorithm is designed such that the producer adds all jobs to the job queue before consumers begin taking them. 
...
Therefore, this implementation qualifies as lock-free.
...
which incurs high costs. Additionally, using locks further decreases overall performance and limits scalability, as described in \secref{concurrency-tool}.
...
\captionof{figure}{The left side shows that the lock protects the entire job queue to ensure exclusive access to its head operations for multi-thread access. 
The right side illustrates that each thread has its own slot for accessing jobs, 
not only achieving exclusivity through data structure but also eliminating the need for shared resources to communicate.}
...
Additionally, consumers don't need the shared resource to communicate.

Following is the interaction between the producer and consumer as shown in \fig{fig:SPMC-Solution3}, 
including their state transitions, is shown as \fig{fig:SPMC-Solution3-FSM}.

\includegraphics[keepaspectratio, width=0.5\linewidth]{images/SPMC-Solution3-FSM}
\captionof{figure}{The finite state machine is as follows: 
When the producer finishes creating jobs, the state transitions from Empty to Task, 
then the producer waits for consumers to complete their tasks. 
When a consumer finishes a task, transitioning from Empty to Done. 
If the total number of completed tasks reaches a certain number, 
the state transitions from Done to End, 
indicating completion. Meanwhile, consumers take jobs, 
transitioning the state from Task to Empty.}
\label{fig:SPMC-Solution3-FSM}
...
We can use \textsc{CAS} with a loop to ensure that the write operation achieves semantic atomicity, as shown in \fig{fig:atomic-type}.
...
it indicates there is another thread is making progress.
...
it doesn't result in an increase in overall throughput. 
...
it doesn't cause other threads to be blocked, thereby preventing overall progress from stalling.
...
take advantage of every concurrency tool or mechanism, and accompany it with appropriate data structures and algorithms. 
